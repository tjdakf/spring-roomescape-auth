...
class ReservationRequestTest {
    ...
    void timeId가_null이면_예외_발생() {
        // when
        Set<ConstraintViolation<ReservationRequest>> result = validator.validate(
                new ReservationRequest(date, null, 1L));
    ...
    void timeId가_양수가_아니면_예외_발생(Long timeId) {
        // when
        Set<ConstraintViolation<ReservationRequest>> result = validator.validate(
                new ReservationRequest(date, timeId, 1L));
    ...
    void themeId가_null이면_예외_발생() {
        // when
        Set<ConstraintViolation<ReservationRequest>> result = validator.validate(
                new ReservationRequest(date, 1L, null));
    ...
    void themeId가_양수가_아니면_예외_발생(Long themeId) {
        // when
        Set<ConstraintViolation<ReservationRequest>> result = validator.validate(
                new ReservationRequest(date, 1L, themeId));
    ...
    void 정상_생성_테스트() {
        ...
        // when
        ReservationRequest result = new ReservationRequest(date, timeId, themeId);
